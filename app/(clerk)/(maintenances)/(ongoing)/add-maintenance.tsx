    ...
    <View style={styles.container}>
      ...
      <ScrollView style={{ width: '100%' }} contentContainerStyle={{ alignItems: 'center' }}>
        ...
        <Dropdown
            ...
            placeholder={ maintenance.equipmentId ? equipments.find(equipment => equipment.id === maintenance.equipmentId)?.name : 'Select Equipment' }
        ...
        <Dropdown
            ...
            valueField="id"
            onChange={(item) => setMaintenance({ ...maintenance, item: item.id })}
            style={styles.dropdown}
            placeholder={ maintenance.item ? items.find(item => item.id === maintenance.item)?.serialNumber : 'Select Item' }
            placeholderStyle={styles.dropdownText}
            selectedTextStyle={styles.dropdownText}
          />
        ...
        <Button title={maintenance.startDate ? maintenance.startDate : "Start Date"} onPress={() => setStartDatePickerVisible(!startDatePickerVisible)} />
        {startDatePickerVisible && (
          <DateTimePicker
            value={ maintenance.startDate ? new Date(maintenance.startDate) : new Date() }
            ...
            onChange={(event, date) => { setStartDatePickerVisible(false); if(date && date.toISOString().split('T')[0] !== maintenance.startDate) { setMaintenance({ ...maintenance, startDate: date.toISOString().split('T')[0] }); } }}
          />
        )}
        </View>
        <View style={{ width: '50%', backgroundColor: 'transparent', alignItems: 'center', justifyContent: 'center' }}>
        <Text style={[styles.text, { marginBottom: '1%' }]}>End Date :</Text>
        <Button title={maintenance.endDate ? maintenance.endDate : "End Date"} onPress={() => setEndDatePickerVisible(!endDatePickerVisible)} />
        {endDatePickerVisible && (
          <DateTimePicker
            value={ maintenance.endDate ? new Date(maintenance.endDate) : new Date() }
            maximumDate={new Date(
              new Date().getFullYear() + 1,
              new Date().getMonth(),
              new Date().getDate()
            ...
            mode='date'
            display='calendar'
            onChange={(event, date) => { setEndDatePickerVisible(false); if(date && date.toISOString().split('T')[0] !== maintenance.endDate) { setMaintenance({ ...maintenance, endDate: date.toISOString().split('T')[0] }); } }}
          />
        )}
        </View>
        </View>
        ...
        <Dropdown
            ...
            labelField="name"
            valueField="id"
            onChange={(item) => setMaintenance({ ...maintenance, assignedTechnician: item.name })}
            style={styles.dropdown}
            placeholder={ maintenance.assignedTechnician ? technicians.find(technician => technician.name === maintenance.assignedTechnician)?.name : 'Select Technician' }
            placeholderStyle={styles.dropdownText}
            selectedTextStyle={styles.dropdownText}
          />
        ...
        <TextInput
          ...
          multiline
          value={maintenance.taskDescription ? maintenance.taskDescription : ''}
          onChangeText={text => setMaintenance({ ...maintenance, taskDescription: text })}
        />
        <View style={styles.separator} />
      </ScrollView>
      </EditSingleItemBackground>
