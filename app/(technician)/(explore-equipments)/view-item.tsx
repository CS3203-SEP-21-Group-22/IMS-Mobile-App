...
export default function ViewItemScreen() {
  ...
  const handleViewReservHistory = ({ item }: { item: Item }) => {
    router.push({ pathname: '/(technician)/(explore-equipments)/reservations', params: { itemId: item.id } });
  }
  const handleViewMaintHistory = ({ item }: { item: Item }) => {
    router.push({ pathname: '/(technician)/(explore-equipments)/maintenances', params: { itemId: item.id } });
  }
  return (
    <BackgroundLayout>
